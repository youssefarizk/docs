...
while True:
    ...
    if status and status['finished']:
        ...
        break
    elif status and status['error'] == False:
...
If you would prefer to provide your own script, you can use the `/scripted` endpoint instead. The payload for that request
...
script_payload = {
    "script": [
        ...
        {"text": "This is the first speaker again again", "voice_id": voice_id_1},
        {"text": "The first speaker likes to talk alot", "voice_id": voice_id_2},
        {"text": "ha ha ha... ok I do", "voice_id": voice_id_1},
    ]
}

response = requests.post(BASE_URL + "/scripted", json=script_payload, headers=headers)
```
