...
To create a new podcast episode, you'll use the `/api/v1/podcast` endpoint with a POST request.
...
response = requests.post(f"{API_BASE_URL}/api/v1/podcast",
                         headers=headers,
...
Once you've started the episode creation process, you can check its status using the `/api/v1/podcast/{job_id}` endpoint with a GET request.
...
def check_episode_status(job_id):
    response = requests.get(f"{API_BASE_URL}/api/v1/podcast/{job_id}", headers=headers)
